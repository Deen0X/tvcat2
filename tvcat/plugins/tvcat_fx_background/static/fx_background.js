/**
 * TVCat FX Background — fondo ambiental del item seleccionado.
 * ES5. Sin filter:blur (downscale por canvas = blur gratis). Doble buffer + crossfade.
 */
(function() {
    var _prefs = { enabled: false, mode: 'lava', crop: 'center', veil: 55, blur: 48, drift_s: 24 };
    var _ready = false;
    var _pending = [];
    ...
    function ensureInit(cb) {
        ...
        ajax({ method: 'GET', url: '/api/fx_background/prefs?device_id=' + encodeURIComponent(getDeviceId()),
            success: function(res) {
                if (res && res.prefs) _prefs = res.prefs;
                ensureLayers();
                _ready = true;
                // Sincronizar el botón del tray con la pref real (ver card_frames).
    ...
    if (window.pluginSystem) {
        window.pluginSystem.registerPlugin({
            name: 'tvcat_fx_background',
            type: 'grid-decorator',
            displayName: 'FX Background',
            // Se auto-aplica en caliente (refreshFxBackground tras el POST):
            ...
            toggleEffect: function() {
                ...
                function applyEnabled() {
                    try {
                        var plg = window.pluginSystem ? window.pluginSystem.getPlugin('tvcat_fx_background') : null;
                        if (plg && plg.refreshFxBackground) plg.refreshFxBackground();
                    } catch (e) {}
                    try { if (window.renderPluginTray) window.renderPluginTray(); } catch (e2) {}
                }
                try {
                    ajax({ method: 'POST', url: '/api/fx_background/prefs',
            ...
            onGridItem: function(element, itemData) {
                ensureInit(function() {
                    if (element.getAttribute('data-fxbg-done')) return;
                    element.setAttribute('data-fxbg-done', '1');
