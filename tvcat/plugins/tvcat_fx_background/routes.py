"""
tvcat_fx_background — plugin grid-decorator de fondo ambiental.

Sin tablas propias: solo prefs por usuario-dispositivo en la DB central
(tvcat_settings, clave fx_background_pref_{user_id}). Todo el efecto es frontend.
"""
...
def _pref_key(user_id) -> str:
    return f"fx_background_pref_{user_id}"
    ...
    try:
        ...
        if row and row[0]:
            ...
            if isinstance(d, dict):
                return d
        # Migración nombre anterior (ambient_pref_*) → fx_background_pref_* (una vez).
        old = conn.execute("SELECT value FROM tvcat_settings WHERE key=?", (f"ambient_pref_{user_id}",)).fetchone()
        if old and old[0]:
            try:
                conn.execute("INSERT OR REPLACE INTO tvcat_settings (key, value) VALUES (?,?)", (_pref_key(user_id), old[0]))
                conn.execute("DELETE FROM tvcat_settings WHERE key=?", (f"ambient_pref_{user_id}",))
                conn.commit()
            except Exception:
                pass
            try:
...
@router.get("/api/fx_background/prefs")
...
@router.post("/api/fx_background/prefs")
