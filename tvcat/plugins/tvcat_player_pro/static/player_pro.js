(function() {
var PRO_DEBUG = true;
function log() { if (PRO_DEBUG) console.log.apply(console, ['[PLAYER_PRO]'].concat(Array.prototype.slice.call(arguments))); }

function getSetting(key, def) {
    var v = localStorage.getItem('tvcat_player_pro_' + key);
...
BackgroundCache.prototype._fetchNext = function() {
    ...
    fetch(this.url, opts).then(function(res) {
...
function overridePlayMedia() {
    if (window.Catalog && window.Catalog.playMedia && window.Catalog.playMedia !== playMediaPro) {
        ...
        window.Catalog.playMedia = function(itemData, episode) {
            if (localStorage.getItem('tvcat_preferred_player') === 'pro') playMediaPro(itemData, episode);
        ...
        window.Catalog.playNext = function() {
            if (localStorage.getItem('tvcat_preferred_player') !== 'pro') return _origPlayNext();
            var eps = window.Catalog.currentEpisodes || {};
        ...
        window.Catalog.playPrevious = function() {
            if (localStorage.getItem('tvcat_preferred_player') !== 'pro') return _origPlayPrev();
            var eps = window.Catalog.currentEpisodes || {};
            var id = window.Catalog.currentMediaId;
            if (!id || !eps[id]) return;
            var mediaData = eps[id];
...
if (window.pluginSystem) {
    window.pluginSystem.registerPlugin({
        name: 'tvcat_player_pro',
        type: 'player',
        displayName: 'Player Pro (Caché Local)',
        playerType: 'pro',
        applies_to: ['media', 'series', 'video', 'anime', 'tv', 'peliculas'],
        action_category: 'playback',
        play: function(item) {
            localStorage.setItem('tvcat_preferred_player', 'pro');
