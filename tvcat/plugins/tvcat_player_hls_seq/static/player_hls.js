(function() {
    ...
    var PLUGIN_NAME = "tvcat_player_hls_seq";
    var LOG_PREFIX = "[PLAYER_HLS]";
    // Debug visible en Tizen 2.4: confirma que el fichero se ha cargado y parseado
    try {
    } catch(e) {}
    ...
    function hlsPlayMedia(item, episodes, episodeIndex) {
        ...
        // Leer prefetch configurado (settings_schema) â€” se envÃ­a al playlist como query param
        var prefetchAhead = 2;
        try {
        ...
        var LANG_ALIASES = {
            spa: ["spa","es","esp","espanol","espaÃ±ol","spanish","castellano"],
            eng: ["eng","en","english","ingles","inglÃ©s"],
            jpn: ["jpn","ja","japanese","japones","japonÃ©s"],
            kor: ["kor","ko","korean","coreano"],
            zho: ["zho","zh","chi","chinese","chino","cmn"],
            fra: ["fra","fre","fr","french","frances","francÃ©s"],
            deu: ["deu","ger","de","german","aleman","alemÃ¡n"],
            ita: ["ita","it","italian","italiano"],
            por: ["por","pt","portuguese","portugues","portuguÃ©s"]
        };
        function normLang(s) {
            if (!s) return "";
            var str = String(s).toLowerCase();
            // WebKit 1.1 (Tizen 2.4) no tiene String.normalize â†’ fallback manual
            if (str.normalize) {
                try { str = str.normalize("NFD").replace(/[\u0300-\u036f]/g,""); } catch(e) {}
            } else {
                str = str.replace(/[Ã Ã¡Ã¢Ã£Ã¤Ã¥]/g,"a").replace(/[Ã¨Ã©ÃªÃ«]/g,"e").replace(/[Ã¬Ã­Ã®Ã¯]/g,"i").replace(/[Ã²Ã³Ã´ÃµÃ¶]/g,"o").replace(/[Ã¹ÃºÃ»Ã¼]/g,"u").replace(/[Ã±]/g,"n").replace(/[Ã§]/g,"c");
            }
                ...
                success: function(hist){
                    try{
                        ...
                        if(resumeTime>5){
                            ...
                            // intentar en varios eventos por si HLS aÃºn no tiene duration
        ...
        // === LOADER HLS (plugin.png rellenÃ¡ndose) ===
        ...
        // ocultar video hasta que estÃ© listo
        ...
        if (!progressBar) {
            ...
            // lÃ­nea blanca de reproducciÃ³n (1px)
        ...
        if(!customLayer){
            ...
            if(_fname.length>80) _fname=_fname.slice(0,80)+"â€¦";
            ...
            // Fila Ãºnica de saltos + play centrados (play a misma altura que saltos, y=46%)
            customLayer.appendChild(createCustomBtn("cc-play-pause",50,46,"play_pause.png","â–¶ï¸","#fff","18px", function(){ try{ if(videoPlayer.paused) videoPlayer.play(); else videoPlayer.pause(); }catch(e){} }));
            ...
            function onTouchStart(e){ if(!customLayer||playerModal.style.display==="none") return; showCustomLayer(); }
            try{
            ...
            window.__hlsSyncCustomCombos=function(){
                try{
                    // En SEQ, los combos se pueblan desde /status (h.audioTracks está vacío porque
                    // el master vía ligera no declara pistas conmutables). No pisar con h.audioTracks.
                    var isSeqMode = (typeof PLUGIN_NAME !== 'undefined' && PLUGIN_NAME.indexOf('_seq') !== -1);
                    var aWrap=document.getElementById("cc-audio-wrap"); var sWrap=document.getElementById("cc-subs-wrap");
                    if(isSeqMode) { if(window.__hlsSeqPopulateCombos) window.__hlsSeqPopulateCombos(window._hlsSeqAudioTracks, window._hlsSeqSubTracks); return; }
            ...
            // SEQ: poblar combos desde las pistas del /status (hls.js ya no las ve como conmutables)
            window.__hlsSeqPopulateCombos=function(audT, subT){
                try{
                    var aSel=document.getElementById("cc-audio-sel"); var sSel=document.getElementById("cc-subs-sel");
                    var aWrap=document.getElementById("cc-audio-wrap"); var sWrap=document.getElementById("cc-subs-wrap");
                    // Poblar SOLO una vez (evita que el polling 1s reescriba el select mientras el usuario despliega)
                    if(aSel && audT && audT.length && aSel.options.length===0){
                        for(var i=0;i<audT.length;i++){ var o=document.createElement("option"); o.value=String(audT[i].idx!=null?audT[i].idx:i); o.textContent=(audT[i].lang||"Audio ")+(audT[i].codec?" ("+audT[i].codec+")":""); aSel.appendChild(o); }
                        // Seleccionar la pista activa/guardada por defecto (evita combo vacío al abrir)
                        var defIdx = 0;
                        try{
                            var pref = (window._hlsTitlePrefs && currentItemId && window._hlsTitlePrefs[currentItemId]) ? window._hlsTitlePrefs[currentItemId] : null;
                            if(pref && pref.audio!=null && pref.audio>=0) defIdx = pref.audio;
                            else if(window._hlsSeqCurrentAudio!=null) defIdx = window._hlsSeqCurrentAudio;
                        }catch(e){}
                        if(aSel.options.length>defIdx) aSel.value=String(audT[defIdx].idx!=null?audT[defIdx].idx:defIdx);
                        if(aWrap) aWrap.style.display=audT.length>1?"block":"none";
                    } else if(aSel && audT && audT.length) {
                        if(aWrap) aWrap.style.display=audT.length>1?"block":"none";
                    }
                    if(sSel && subT && sSel.options.length===0){
                        var off=document.createElement("option"); off.value="-1"; off.textContent="Sin subs"; sSel.appendChild(off);
                        for(var j=0;j<subT.length;j++){ var o2=document.createElement("option"); o2.value=String(subT[j].idx!=null?subT[j].idx:j); o2.textContent=(subT[j].lang||("Sub "+j)); sSel.appendChild(o2); }
                        if(sWrap) sWrap.style.display=subT.length>0?"block":"none";
                    } else if(sSel && subT) {
                        if(sWrap) sWrap.style.display=subT.length>0?"block":"none";
                    }
                }catch(e){}
        ...
        // Polling del estado de descarga + punto de reproducciÃ³n + selectores
        ...
        function ensureTrackSelectors(audioTracks, subTracks, extSubs) {
            ...
            if ((subTracks && subTracks.length > 0) || (extSubs && extSubs.length > 0)) {
                ...
                _subsSel.onchange = function() {
                    var v = this.value;
                    log("Sub seleccionado (Ã­ndice master): " + v);
                    try {
                ...
                // Llenar combo usando el Ã­ndice real de hls.subtitleTracks
        ...
        function pollCacheStatus() {
            ...
            // si el modal ya estÃ¡ cerrado, parar el polling
            ...
            var isSeq = (typeof PLUGIN_NAME !== 'undefined' && PLUGIN_NAME.indexOf('_seq') !== -1);
            var statusUrl = isSeq ? "/api/hls_seq/" + episodeKey + "/status" : "/api/hls/" + episodeKey + "/cache-status";
            window.API.ajax({
                url: statusUrl,
                success: function(data) {
                    if(isSeq){
                        // SEQ: barra verde = progreso del servicio central (bytes_done / bytes_total)
                        if(islandsLayer && data && data.bytes_total > 0){
                        ...
                        if(data && (data.audio_tracks || data.sub_tracks)){
                            ...
                            try{ if(window.__hlsSeqPopulateCombos) window.__hlsSeqPopulateCombos(data.audio_tracks, data.sub_tracks); }catch(e){}
                            try{ ensureTrackSelectors(data.audio_tracks, data.sub_tracks || [], []); }catch(e){}
                        }
                        // En SEQ el audio NO se sincroniza con h3.audioTrack (vía ligera: master sin
                        // pistas conmutables, audioTrack es undefined). NO pisar el combo de audio.
                        try{
                            var h3 = videoPlayer._hls;
                            if(h3){
                                // Subs: solo sincronizar si real >=0 (evita pisar la selección manual mientras hls.js aplica)
                                var csSel2=document.getElementById("cc-subs-sel"); if(csSel2 && h3.subtitleTrack!=null && h3.subtitleTrack>=0 && csSel2.options.length>0) csSel2.value=String(h3.subtitleTrack);
                            }
                        }catch(e){}
                        // Actualizar punto blanco de reproducción (playDot) en SEQ
                        try{
                            var durS = videoPlayer.duration || 0;
                            var curS = videoPlayer.currentTime || 0;
                            var playDotSeq = document.getElementById("hls-play-dot");
                            if(durS > 0 && playDotSeq){
                                var ppS = Math.min(100, Math.max(0, (curS / durS) * 100));
                                playDotSeq.style.left = ppS + "%";
                            }
                        }catch(e){}
                        return;
                    }
                    ...
                    if (data && (data.audio_tracks || data.sub_tracks || data.subs)) {
                        ...
                        try {
                            ...
                            if (hasTracks && !hlsHasTracks && !window.__hlsMasterReloaded) {
                                ...
                                // intentar auto-select si aÃºn no aplicado
                                try{ if(window.__doAutoSelect) window.__doAutoSelect(); }catch(e){}
                            }
                        } catch(e2) {}
                    }
        ...
        function notifyLeave() {
            ...
            try{
                if(customLayer){
                    ...
                    // quitar del DOM para no bloquear clicks en catÃ¡logo
        ...
        // MODO SOLO DESCARGA (diagnÃ³stico): no reproduce, solo descarga el sparse y muestra la barra
        ...
        // Reset completo para segunda reproducciÃ³n y seek
        try { videoPlayer.pause(); } catch(e) {}
        ...
        // Si HLS aÃºn no estÃ¡ cargado y nativo no sirve, cargarlo bajo demanda (no bloquea registro en WebKit vieja)
        var needHls = (typeof Hls === "undefined");
        if (needHls) {
            // intentar carga diferida solo si hace falta hls.js; mientras tanto seguir con nativo si es posible
            // no bloquea el registro, solo el playback: ensureHls cargarÃ¡ hls.min.js y reevaluarÃ¡
        }
        ...
        if (canHlsJs) {
            ...
            var hls = new Hls({
                ...
                backBufferLength: 90,
                maxBufferLength: 60,        // buffer objetivo 60s por delante (reduce cortes)
                maxMaxBufferLength: 120,     // tope 120s si la máquina lo permite
                maxBufferSize: 90*1000*1000, // ~90MB buffer
                fragLoadingTimeOut: 15000,   // 15s por segmento (generación local, no red)
            ...
            window.__doAutoSelect = function(){
                try{
                    // Prio por tÃ­tulo (central) prevalece sobre global
                    ...
                    if(titlePref){
                        ...
                        // si tÃ­tulo solo tenÃ­a una de las dos, dejar que global resuelva la otra
                    }
            ...
            // SEQ: aplicar audio/subs por preferencia (config global o por título). En vía ligera
            // hls.audioTracks está vacío, así que se resuelve con audio_tracks del /status y se
            // recarga el media con ?audio=N si difiere de la pista activa.
            window.__hlsSeqApplyAudio = function(){
                try{
                        ...
                        if(h){
                            ...
                            var onP2 = function(){
                                try{
                                    ...
                                    else if(sIdx2<0){ h.subtitleTrack = -1; }
                                    var buscar=function(){ try{ if(curSec>0 && Math.abs(videoPlayer.currentTime-curSec)>0.3){ videoPlayer.currentTime=curSec; h.startLoad(); } videoPlayer.play(); }catch(e){} };
                                    setTimeout(buscar,200); setTimeout(buscar,600); setTimeout(buscar,1200);
                                }catch(e){}
                                try{ h.off(Hls.Events.MANIFEST_PARSED, onP2); }catch(e){}
                            };
        ...
        try{
            var lastSavedPos=-1;
            function saveHlsProgress(completed){
                try{
            ...
            // guardar al salir (envolver notifyLeave ya lo hace, aÃ±adir save)
    ...
    function init() {
        ...
        try {
            ...
            _ok.style.cssText = 'position:fixed;top:36px;left:0;background:#22c55e;color:#fff;font-size:10px;padding:2px 6px;z-index:999999;';
            if (document.body) document.body.appendChild(_ok);
            setTimeout(function(){ try{ _ok.remove(); }catch(e){} }, 3000);
        } catch(e) {}
        } catch(e) {
            try {
                var _err = document.createElement('div');
                _err.style.cssText = 'position:fixed;top:36px;left:0;background:red;color:#fff;font-size:10px;padding:2px 6px;z-index:999999;';
                if (document.body) document.body.appendChild(_err);
            } catch(e2) {}
        }
    }
