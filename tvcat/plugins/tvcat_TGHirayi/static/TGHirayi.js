...
html += '</div>';
    ...
    function renderJobRow(j, current, isDone) {
        ...
        if (((j.in_scope_total || j.total_episodes) || 0) > 1) {
            h += '<span onclick="window._tgcopyEpisodes(\'' + j.id + '\')" title="Ver episodios y elegir cuáles copiar" style="cursor:pointer;font-size:11px;background:rgba(59,130,246,0.15);color:#93c5fd;border:1px solid rgba(59,130,246,0.4);border-radius:4px;padding:1px 5px;white-space:nowrap;">🎞️ ' + (j.in_scope_total || j.total_episodes) + '</span>';
        }
        if (j.is_archive) {
