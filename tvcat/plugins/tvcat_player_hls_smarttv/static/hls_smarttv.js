(function() {
    ...
    var PLUGIN_NAME = "tvcat_player_hls_smarttv";
    ...
    // FASE 2: Detección mando vs teclado (posición) + toggle Forzar mando
    function isForcedRemote() { try { return localStorage.getItem('tvcat_force_remote') === 'true'; } catch(e) { return false; } }
    function isRemoteEvent(e) {
        if (isForcedRemote()) return true;
        try { var cap = window.detectDeviceCapabilities ? window.detectDeviceCapabilities() : (window.Catalog && window.Catalog.detectDeviceCapabilities ? window.Catalog.detectDeviceCapabilities() : null); if (cap && cap.isOldSmartTV) return true; } catch(e2) {}
        if (!e) return false;
        var code = e.code || "";
        if (code.indexOf("Numpad") === 0 || code.indexOf("Digit") === 0) return false;
        if (e.location === 3) return false;
        if (code === "" || code === "Unidentified") return true;
        return false;
    }
    function translateRemoteDigit(d) {
        var m = {"1":"7","2":"8","3":"9","7":"1","8":"2","9":"3"};
        return m[d] || d;
    }
    ...
    function hlsPlayMedia(item, episodes, episodeIndex) {
        ...
        // FASE 1: tamaño responsive — TV vieja mantiene 100vh (plane HW), PC usa maxWidth+aspectRatio fluido
        var _isOldTvSize = false;
        try { var _cS = window.detectDeviceCapabilities ? window.detectDeviceCapabilities() : (window.Catalog && window.Catalog.detectDeviceCapabilities ? window.Catalog.detectDeviceCapabilities() : null); _isOldTvSize = _cS && _cS.isOldSmartTV; } catch(e){}
        var _isFsSize = !!(document.fullscreenElement || document.webkitFullscreenElement);
        if (pContainer) {
            if (_isOldTvSize || _isFsSize) {
                pContainer.style.display = "flex";
                pContainer.style.background = "#000";
                pContainer.style.width = "100%";
                pContainer.style.height = "100vh";
                pContainer.style.maxWidth = "100%";
                pContainer.style.aspectRatio = "";
            } else {
                pContainer.style.display = "block";
                pContainer.style.background = "#000";
                pContainer.style.width = "100%";
                pContainer.style.maxWidth = "960px";
                pContainer.style.height = "auto";
                pContainer.style.aspectRatio = "16 / 9";
        ...
        if(!customLayer){
            ...
            var onKeyDown = function(e){
                ...
                log("onKeyDown raw kc="+(e.keyCode||e.which)+" key="+e.key+" code="+(e.code||"")+" curLayer="+(!!curLayer)+" modalDisplay="+(curModal?curModal.style.display:"?"));
                ...
                if(d===null) return;
                // FASE 2: discriminar mando (posición) vs teclado -> traducir dígito físico a lógico
                try { if (isRemoteEvent(e)) d = translateRemoteDigit(d); } catch(e2) {}
            ...
            if(!window._hlsKeyHandlerRegistered){
                try{ window.addEventListener("keydown", onKeyDown, true); document.addEventListener("keydown", onKeyDown, true); window._hlsKeyHandlerRegistered=true; log("key handler registered capture"); }catch(e){}
                // Gamepad polling: DPAD=8246, A=5 B=0 X=2 Y=8 L1=7 R1=9 L2=1 R2=3
                try {
                    var _gpPrev = {};
                    setInterval(function(){
                        try {
                            ...
                            var gp = null;
                            for (var gi=0; gi<pads.length; gi++) if (pads[gi]) { gp = pads[gi]; break; }
                            if (!gp) return;
                            var curLayer2 = document.getElementById("hls-custom-layer");
                            var curModal2 = document.getElementById("player-modal");
                            if (!curLayer2 || !curModal2 || curModal2.style.display==="none") return;
                            var now = Date.now();
                            function gpTrig(dig) {
                                if (_gpPrev[dig] && now - _gpPrev[dig] < 300) return;
                                _gpPrev[dig] = now;
                                var fake = {key: dig, keyCode: dig.charCodeAt(0), which: dig.charCodeAt(0), code: ""};
                                onKeyDown(fake);
                            }
                            if (gp.buttons[12] && gp.buttons[12].pressed) gpTrig("8");
                            else if (gp.buttons[13] && gp.buttons[13].pressed) gpTrig("2");
                            else if (gp.buttons[14] && gp.buttons[14].pressed) gpTrig("4");
                            else if (gp.buttons[15] && gp.buttons[15].pressed) gpTrig("6");
                            else if (gp.axes && gp.axes[1] < -0.5) gpTrig("8");
                            else if (gp.axes && gp.axes[1] > 0.5) gpTrig("2");
                            else if (gp.axes && gp.axes[0] < -0.5) gpTrig("4");
                            else if (gp.axes && gp.axes[0] > 0.5) gpTrig("6");
                            if (gp.buttons[0] && gp.buttons[0].pressed) gpTrig("5");
                            if (gp.buttons[1] && gp.buttons[1].pressed) gpTrig("0");
                            if (gp.buttons[2] && gp.buttons[2].pressed) gpTrig("2");
                            if (gp.buttons[3] && gp.buttons[3].pressed) gpTrig("8");
                            if (gp.buttons[4] && gp.buttons[4].pressed) gpTrig("7");
                            if (gp.buttons[5] && gp.buttons[5].pressed) gpTrig("9");
                            if (gp.buttons[6] && (gp.buttons[6].pressed || gp.buttons[6].value > 0.5)) gpTrig("1");
                            if (gp.buttons[7] && (gp.buttons[7].pressed || gp.buttons[7].value > 0.5)) gpTrig("3");
                        } catch(e) {}
                    }, 150);
                } catch(e) {}
            }
    ...
    function init() {
        ...
        try {
            ...
            name: PLUGIN_NAME,
            type: "player",
            displayName: "Player HLS SmartTV",
            playerType: "hls_smarttv",
            version: "1.0.0",
