    ...
    try:
        ...
        for attr in getattr(doc, 'attributes', []) or []:
            ...
            if fn:
                fname = fn.lower()
                break
        if fname.endswith(('.mp4', '.m4v', '.mov')):
            return True
        if fname.endswith(('.avi', '.mkv', '.webm', '.flv', '.wmv')):
            return False
    except Exception:
        pass
    return True
...
async def _hls_worker_loop():
    ...
    try:
        while not _HLS_WORKER_STOP:
            ...
            # Pausar si se solicitó leave (last_active==0) o lleva >30s sin segmento
            ...
            if _last == 0:
                ...
                continue
            if (_t.time() - _last) > 30:
                printLog(f" [HLS-WORKER] ep={episode_key} pausado por inactividad ({_t.time()-_last:.0f}s sin segmento)")
                _HLS_WORKER_QUEUE.pop(0)
                await _hls_close_secondary(secondary)
                secondary = []
                current_ep = None
            ...
            block = _hls_next_missing_block(state, prefer_from=pb)
            if block is None:
                _HLS_WORKER_QUEUE.pop(0)
                await _hls_close_secondary(secondary)
                secondary = []
...
async def hls_subs_embed_segment(episode_key: str, idx: int, n: int):
    ...
    # recortar al intervalo PERO mantener tiempos ABSOLUTOS
    out = ["WEBVTT", ""]
    for s, e, txt in cues:
        if e <= seg_start or s >= seg_end:
            continue
        # MANTENER TIEMPOS ABSOLUTOS (s y e)
        out.append(f"{_fmt_vtt_time(s)} --> {_fmt_vtt_time(e)}")
        out.append(txt)
        out.append("")
    if len(out) <= 2:
        out = ["WEBVTT", ""]
...
_USER_PREF_KEYS = ("display_name", "avatar", "avatar_url", "color", "category_preferences", "watch_threshold_min", "watch_threshold_max")
